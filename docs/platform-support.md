...
| `factory` | yes | yes | Manifest-driven factory workflow that turns a high-level request into intake artifacts, a validated factory-plan.json, story manifests, validation manifests, queue ledgers, dispatch dry-runs, integration gates, release status, and static evidence hubs without default AI dispatch. |
