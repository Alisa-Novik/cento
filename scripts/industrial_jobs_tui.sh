...
BINARY="$CACHE_DIR/cento-industrial-jobs-tui"
SOURCE_FILE="$ROOT_DIR/scripts/industrial_jobs_tui.go"
...
if [[ ! -x "$BINARY" || "$SOURCE_FILE" -nt "$BINARY" || "$GO_MOD" -nt "$BINARY" || ( -f "$GO_SUM" && "$GO_SUM" -nt "$BINARY" ) ]]; then
    (cd -- "$ROOT_DIR" && go build -o "$BINARY" ./scripts/industrial_jobs_tui.go)
fi
