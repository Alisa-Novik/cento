...
FOUNDRY_TEMPLATE_ROOT = ROOT_DIR / "templates" / "foundry"
CLIENT_INTAKE_HUB_TEMPLATE_ROOT = FOUNDRY_TEMPLATE_ROOT / "client-intake-hub"
CLIENT_INTAKE_HUB_DOCS_PATH = ROOT_DIR / "docs" / "client-intake-hub.md"
...
def safe_foundry_static_path(request_path: str) -> Path:
    raw = unquote(request_path.removeprefix("/foundry/"))
    rel_path = Path(raw)
    if rel_path.is_absolute() or any(part in {"", ".", ".."} for part in rel_path.parts):
        raise CRMError("Invalid Foundry asset path.")
    target = FOUNDRY_TEMPLATE_ROOT / rel_path
    resolved = target.resolve()
    root = FOUNDRY_TEMPLATE_ROOT.resolve()
    if not resolved.is_relative_to(root) or not resolved.is_file():
        raise FileNotFoundError(raw)
    return resolved


def foundry_tools_payload() -> dict[str, Any]:
    bundle_files = [
        CLIENT_INTAKE_HUB_TEMPLATE_ROOT / "client-intake-hub.html",
        CLIENT_INTAKE_HUB_TEMPLATE_ROOT / "client-profile.schema.json",
        CLIENT_INTAKE_HUB_TEMPLATE_ROOT / "command-api.json",
        CLIENT_INTAKE_HUB_TEMPLATE_ROOT / "storage-leak-policy.json",
        CLIENT_INTAKE_HUB_TEMPLATE_ROOT / "validation-plan.json",
        CLIENT_INTAKE_HUB_TEMPLATE_ROOT / "README.md",
    ]
    materialized = all(path.exists() for path in bundle_files) and CLIENT_INTAKE_HUB_DOCS_PATH.exists()
    return {
        "schema_version": "cento.crm.foundry_tools.v1",
        "tools": [
            {
                "id": "client-intake-hub",
                "title": "Client Intake Hub",
                "status": "materialized" if materialized else "not_materialized",
                "description": "Fixture-only career consulting intake bundle generated by Cento Tool Foundry.",
                "target_root": repo_rel(CLIENT_INTAKE_HUB_TEMPLATE_ROOT),
                "docs_path": repo_rel(CLIENT_INTAKE_HUB_DOCS_PATH),
                "preview_path": "/foundry/client-intake-hub/client-intake-hub.html",
                "privacy": "fixture-only; no real client data",
                "files": [repo_rel(path) for path in bundle_files if path.exists()],
            }
        ],
    }
        ...
        def do_GET(self) -> None:
            try:
                ...
                if parsed.path == '/api/request-log':
                    ...
                    return
                if parsed.path == '/api/foundry/tools':
                    json_response(self, 200, {"ok": True, **foundry_tools_payload()})
                    return
                ...
                if parsed.path == '/docs':
                    ...
                    return
                if parsed.path.startswith('/foundry/'):
                    target = safe_foundry_static_path(parsed.path)
                    file_response(self, 200, target.read_bytes(), content_type_for(target.name))
                    return
