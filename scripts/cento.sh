...
TMUX_INIT_FILE="$CONFIG_DIR/tmux.conf"
TMUX_CONF_FILE="$HOME/.tmux.conf"
TMUX_INIT_START="# >>> cento tmux >>>"
TMUX_INIT_END="# <<< cento tmux <<<"
...
if [[ -o interactive ]]; then
  case "\$(uname -s)" in
    Darwin) _cento_node_platform=macos ;;
    Linux) _cento_node_platform=linux ;;
    *) _cento_node_platform=\$(uname -s | tr '[:upper:]' '[:lower:]') ;;
  esac
  _cento_node_host=\${HOST:-\$(hostname 2>/dev/null || printf unknown)}
  _cento_node_host=\${_cento_node_host%%.*}
  export CENTO_NODE_PLATFORM=\${CENTO_NODE_PLATFORM:-\$_cento_node_platform}
  export CENTO_NODE_LABEL=\${CENTO_NODE_LABEL:-cento:\$CENTO_NODE_PLATFORM:\$_cento_node_host}

  if [[ -z "\${CENTO_PROMPT_MARKER_INSTALLED:-}" ]]; then
    export CENTO_PROMPT_MARKER_INSTALLED=1
    if [[ -n "\${RPROMPT:-}" ]]; then
      RPROMPT="%F{39}[\$CENTO_NODE_LABEL]%f \$RPROMPT"
    else
      RPROMPT="%F{39}[\$CENTO_NODE_LABEL]%f"
    fi
  fi
fi
EOF_ZSH
...
write_tmux_init() {
    local platform host label
    platform=$(tmux_node_platform)
    host=$(hostname 2>/dev/null || printf unknown)
    host=${host%%.*}
    label="cento:${platform}:${host}"

    cento_ensure_dir "$CONFIG_DIR"
    cat > "$TMUX_INIT_FILE" <<EOF_TMUX
# Generated by cento install tmux
set -g @cento_node_label "$label"
set -g status on
set -g status-left-length 48
set -g status-right-length 96
set -g status-left "#[fg=colour39,bold][#S] #[fg=colour45]$label #[default]"
set -g status-right "#[fg=colour244]%Y-%m-%d %H:%M #[fg=colour39]#H"
set-window-option -g window-status-current-format "#[fg=colour39,bold]#I:#W#{?window_flags,#{window_flags},}"
set-window-option -g window-status-format "#[fg=colour244]#I:#W#{?window_flags,#{window_flags},}"
EOF_TMUX
}

ensure_tmux_conf_sources_cento() {
    touch "$TMUX_CONF_FILE"
    if grep -Fq "$TMUX_INIT_START" "$TMUX_CONF_FILE"; then
        return 0
    fi

    cat >> "$TMUX_CONF_FILE" <<EOF_TMUX_CONF

$TMUX_INIT_START
if-shell 'test -f "$TMUX_INIT_FILE"' 'source-file "$TMUX_INIT_FILE"'
$TMUX_INIT_END
EOF_TMUX_CONF
}
...
install_tmux_integration() {
    write_tmux_init
    ensure_tmux_conf_sources_cento
    if [[ -n "${TMUX:-}" ]] && cento_have_cmd tmux; then
        tmux source-file "$TMUX_CONF_FILE" || true
        tmux refresh-client -S || true
    fi
    printf 'Installed cento tmux integration.\n'
    printf 'Tmux init: %s\n' "$TMUX_INIT_FILE"
...
handle_install() {
    ...
    case "$target" in
        ...
        tmux)
            ...
            ;;
        *)
            cento_die "Usage: cento install [all|zsh|tmux]"
            ;;
    esac
}
