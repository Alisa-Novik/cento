...
BINARY="$CACHE_DIR/cento-agent-processes-tui"
SOURCE_FILE="$ROOT_DIR/scripts/agent_processes_tui.go"
...
if [[ ! -x "$BINARY" || "$SOURCE_FILE" -nt "$BINARY" || "$GO_MOD" -nt "$BINARY" || ( -f "$GO_SUM" && "$GO_SUM" -nt "$BINARY" ) ]]; then
    (cd -- "$ROOT_DIR" && go build -o "$BINARY" ./scripts/agent_processes_tui.go)
fi
